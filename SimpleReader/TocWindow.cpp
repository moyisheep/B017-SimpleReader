...
TocPanel::TocPanel()
{
    ...
    m_hFont = CreateFontW(18, 0, 0, 0,
        ...
        m_font_family.c_str());   // 字体名

    // 1. 在 WM_CREATE / 初始化时创建一次

    m_hightlightBrush = CreateSolidBrush(m_highlight_color);
    m_hoverBrush = CreateSolidBrush(m_hover_color);
...
void TocPanel::SetHighlight(size_t target)
{
    // ---------- 1. 找目标节点 ----------
