...
// ---------- LiteHtml 容器 ----------
class d2dContainer : public litehtml::document_container {
public:
    d2dContainer(int w, int h, HWND hwnd);
    ~d2dContainer();
    ...
    static std::string trim_any(const std::string& s, const char* ws);
    ImgFmt detect_fmt(const uint8_t* d, size_t n, const char* ext);
    ImageFrame decode_img(const std::vector<uint8_t>& mf, const char* ext);
